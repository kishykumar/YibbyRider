...
class BaseYibbyTableViewController: UITableViewController {
    ...
    override func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if let menuButton = self.view.viewWithTag(MENU_BUTTON_TAG) as? UIButton {
            
            let app: UIApplication = UIApplication.shared
            menuButton.frame = CGRect(x: menuButton.frame.x, y: scrollView.contentOffset.y + app.statusBarFrame.size.height, width: menuButton.bounds.size.width, height: menuButton.bounds.size.height)
        }
