...
- (void)updatePopUpView
{
    ...
    dispatch_once(&once, ^ {
        NSLayoutConstraint *verticalConstraint = [NSLayoutConstraint
                                                  constraintWithItem:self.popUpView
                                                  attribute:NSLayoutAttributeTop
                                                  relatedBy:NSLayoutRelationEqual
                                                  toItem:self.popUpView.superview.superview
                                                  attribute:NSLayoutAttributeTop
                                                  multiplier:1.0
                                                  constant:5];
        
        [self.popUpView.superview.superview addConstraint:verticalConstraint];
    });
}
...
- (CGRect)thumbRect
{
    CGRect trackRectBounds = [self trackRectForBounds:self.bounds];
    CGRect thumbRectBounds = [self thumbRectForBounds:self.bounds
                        trackRect:trackRectBounds
                        value:self.value];
    return thumbRectBounds;
}
...
- (CGRect)trackRectForBounds:(CGRect)bounds
{
    ...
    CGRect oldBounds = [super trackRectForBounds:bounds];
    CGRect newBounds = oldBounds;
    
    // 1.5% of screen size. I know there has to be a better way, but in vain right now.
    newBounds.size.height = screenSize.size.height * 0.015;
    
    newBounds.origin.y = newBounds.origin.y - ((newBounds.size.height - oldBounds.size.height) / 2);
